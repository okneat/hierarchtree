...
import io.search.config.KafkaProperties
...
class HierarchtreeKafkaResourceIT {
    ...
    fun setup() {
        ...
        val kafkaResource = HierarchtreeKafkaResource(kafkaProperties)
    ...
    fun producesMessages() {
        restMockMvc.perform(post("/api/hierarchtree-kafka/publish/topic-produce?message=value-produce"))
    ...
    fun consumesMessages() {
        ...
        val mvcResult = restMockMvc.perform(get("/api/hierarchtree-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
